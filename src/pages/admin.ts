...
async function renderDashboard(container: HTMLElement) {
  ...
  function attachProductActions() {
    ...
    document.querySelectorAll(".edit-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        targetBtn.innerText = "Carregando...";
        ...
        try {
          ...
          modal.classList.remove("hidden");
           ...
           console.error(err);
           alert("Erro ao carregar os dados completos do produto.");
        } finally {
  ...
  fileInput.addEventListener("change", async () => {
    ...
    for (const file of files) {
      ...
      uploadedBase64Images.push(compressedBase64);
    }
        ...
        for (const base64 of uploadedBase64Images) {
          try {
            ...
            if (base64.startsWith('data:image')) {
              const url = await store.uploadImage(base64, 'image.webp');
              finalUrls.push(url);
            } else {
            ...
            alert("Falha no salvar a imagem.");
            throw err;
          }
        }
      } else {
...
/**
 * Compresses a base64 image on the client side to reduce upload size and bandwidth.
 */
async function compressImage(base64Str: string, maxWidth = 1000, maxHeight = 1000, quality = 0.8): Promise<string> {
