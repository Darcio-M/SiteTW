...
export async function renderProduct(container: HTMLElement, id: string) {
  ...
  try {
    ...
    container.innerHTML =
      '<div class="text-center py-32"><div class="inline-flex w-16 h-16 bg-white border border-red-200 text-red-400 rounded-full items-center justify-center mb-4"><i data-lucide="alert-circle" class="w-8 h-8"></i></div><h3 class="text-lg font-serif text-sleek-text mb-2 text-red-600">Problemas Técnicos</h3><p class="text-sleek-text-light font-sans font-light text-sm">Não foi possível carregar os detalhes do produto no momento.</p></div>';
  ...
  if (!product) {
    container.innerHTML =
      '<div class="text-center py-32"><h2 class="text-2xl text-gray-400">Produto não encontrado.</h2></div>';
    return;
  }

  const categories = await store.getCategories();
  const categoryName =
    categories.find((c) => c.id === product.category)?.name || "Outros";
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div class="flex flex-col justify-center">
            ...
            <div id="priceTiersInfo" class="text-xs font-sans text-sleek-text-light mb-8">
  ...
  document
    ...
    ?.addEventListener("click", () => updateImage(currentImageIdx - 1));
  document
  ...
  const phoneNumber = "558892544294";
  ...
  function updatePriceAndLink() {
    ...
    let s = product.sizes && product.sizes.length > 0 ? product.sizes[0] : null;
    ...
    let matchedTierPrice: number | null = null;
    let tiersHtml = `Mínimo para compra: ${minQ} un.<br>`;
    
    if (s && s.priceTiers && s.priceTiers.length > 0) {
      const sortedTiers = [...s.priceTiers].sort((a, b) => b.quantity - a.quantity);
      
      const tiersListHtml = [...s.priceTiers].sort((a, b) => a.quantity - b.quantity).map(t => 
         `Acima de ${t.quantity} un.: R$ ${t.price.toFixed(2).replace(".", ",")}/un.`
      ).join(' | ');
      
      tiersHtml += `<span class="text-sleek-accent">Atacado: ${tiersListHtml}</span>`;

      for (const tier of sortedTiers) {
        if (qty >= tier.quantity) {
          matchedTierPrice = tier.price;
          break;
        }
      }
    } else if (wholesaleP !== baseP) {
      tiersHtml += `<span class="text-sleek-accent">Atacado a partir de ${wholesaleMinQ} un.: R$ ${wholesaleP.toFixed(2).replace(".", ",")}/un.</span>`;
    }

    const tiersInfoEl = document.getElementById("priceTiersInfo");
    if (tiersInfoEl) {
      tiersInfoEl.innerHTML = tiersHtml;
    }

    if (qty < minQ) {
      labelText = "Tratar com o vendedor:";
      currentPrice = baseP;
    } else if (matchedTierPrice !== null) {
      currentPrice = matchedTierPrice;
      labelText = "Valor (Atacado):";
    } else if (qty >= wholesaleMinQ && wholesaleP !== baseP && (!s || !s.priceTiers || s.priceTiers.length === 0)) {
    ...
    buyBtn.onclick = (e) => {
      if (qty <= 0 || !cep || !contact) {
        ...
        errorMsg.textContent =
          "Preencha o Contato, CEP e a quantidade (mínima de 1) para continuar.";
